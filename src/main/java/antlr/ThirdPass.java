...
import java.util.ArrayList;
	...
	public class ThirdPass {
	    ...
	    public static ArrayList<AffectationStatique> listeAffectationStatique = new ArrayList<AffectationStatique>();

	    private FileWriter writer;

	    public ThirdPass(boolean debug, FileWriter fw, EtiquetteGenerator etiquetteGenerator) {
	        this.debug = debug;
	        this.writer = fw;
	    ...
	    public void launch(Tree tree, TDS tds) throws IOException {
	        thirdPass(tree, tds);
	        if(debug){
	            System.out.println("third pass OK");
	        }
	    }

	    public void thirdPass(Tree t, TDS tds) throws IOException{
	    	OperateurCondition c = new OperateurCondition(writer);
	    	String resultRegister = "R6";
	        switch (t.getType()){
	            case 31: //root node
	                for(int i = 0; i<t.getChildCount();i++){
	                    thirdPass(t.getChild(i), tds);
	                }
	                break;
...
//	                tds_counter_3rd_pass--;
	            ...
	            case GrammaireParser.ATTRIBUT: //variable de classe
	            	for(int i=0;i<t.getChildCount();i++) {
	            		if(t.getChild(i).getType()==GrammaireParser.VALUE) {
	            			if(t.getChild(t.getChildCount()-1).getText().contentEquals("static")) {
	            				listeAffectationStatique.add(new AffectationStatique(t, tds, currentClass));
	            			}
	            ...
	            case GrammaireParser.METHOD:
	            	...
	            	writer.write("//methode "+currentMethod+"\n"+
	            			currentClass+"_"+currentMethod+"\n");
	            	TasASM.ajoutAdressesMethodes(currentClass, currentMethod);
	                ...
	                writer.write(ToolsASM.RTS()+"//fin methode "+currentMethod+"\n");
	                currentMethod=null;
	                break;

	            case GrammaireParser.PARAM: // PAS A FAIRE CAR DEJA FAIT DANS LIST_PARAM

	                thirdPass(t.getChild(0), tds);
	                thirdPass(t.getChild(1), tds);
	            ...
	            case GrammaireParser.AFFECTATION:
	                ...
	                //thirdPass(t.getChild(0));
	                //thirdPass(t.getChild(1));
...
//	                }
	            ...
	            case GrammaireParser.WHILE:
					...
					thirdPass(t.getChild(0), tds); //condition
					if(!currentClass.contentEquals("main")) {
						if____.writer_condition_code(currentClass);
					}
					thirdPass(t.getChild(1), tds); //do
					if(!currentClass.contentEquals("main")) {
	            ...
	            case GrammaireParser.APPEL_FONCTION:
	                if(t.getChild(0).getText().equals("super")){
	                    ...
	                    thirdPass(t.getChild(0).getChild(0), tds);
	                    if(!currentClass.contentEquals("main")) {
		                    Printer p = new Printer(writer);
...
//		                        }
		                    ...
		                    else if(t.getChild(0).getChild(0).getText().equals("println")){ //same for println
...
//		                        }
	            ...
	            case GrammaireParser.SELECT:
	                thirdPass(t.getChild(0).getChild(0), tds);
	                thirdPass(t.getChild(0).getChild(1), tds);
	                if(t.getChild(0).getType() == GrammaireParser.MESSAGE){
	                	Tree t_ = t.getChild(0);
						if(t_.getChild(t_.getChildCount()-1).getType()==GrammaireParser.APPEL_FONCTION && !currentClass.contentEquals("main")) {
							Printer p = new Printer(writer);
							if(!p.esperluateConcat_check(t_.getChild(0))) {
								ElementObjet obj = new ElementObjet(currentClass, t, "Objet" );
//								//on a pas besoin du resultat dans R0 mais ce n'est pas grave si il y est
//								//donc on appelle cette fonction qui fait exactement ce que l'on veut
								TasASM.attributDansR0(tds, obj, -1, false);
...
//							tds.getTDSNumRegion(tds_counter_3rd_pass),
//							"R1");
//					m.makeOperation(t.getChild(0), "R1",
//							t.getChild(1), "R2",
//							false);
//					if (t.getParent().getType() == GrammaireParser.AFFECTATION) {
//						if (t.getParent().getChild(0).getType() == GrammaireParser.VARIABLE) {
//							ToolsASM.changerVariableEtEcrire(writer,
//									tds.getTDSNumRegion(tds_counter_3rd_pass),
//									t.getParent().getChild(0).getChild(0).getText(),
//									"R1");
//						}
//					}
					...
					if (ClimbUp.climbUp(t).getType() == GrammaireParser.AFFECTATION) {
						if (ClimbUp.climbUp(t).getChild(0).getType() == GrammaireParser.VARIABLE) {
							ToolsASM.changerVariableEtEcrire(writer,
									...
									resultRegister);
						} else if (ClimbUp.climbUp(t).getChild(0).getType() == GrammaireParser.IDENTIFICATEUR) {
							ElementObjet obj = new ElementObjet(currentClass,
									ClimbUp.climbUp(t).getChild(0).getChild(0),
									"Objet" );
